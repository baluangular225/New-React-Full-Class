...
const Usestate105 = () =>{
 ...
 const handleDelete = async (id) =>{
    try {
        ...
        console.log('Error Message Employee Delete', error)
    }
 }
 ...
 const handleEdit = async (id, name, email, website) =>{
    ...
    .then(response=>{
        if(!response.ok){
            throw new Error('Failed Employee Editing')
        }
    })
    .catch(error=>{
        console.log('Error Message', error)
 ...
 const updateData = async () =>{
   ...
   setErrorMessage(false)
    ...
    try {
        ...
        const Todos = employee.map((eachEmployee)=>{
            if(eachEmployee.id === editId){
                return{...eachEmployee, ...allEmployee}
            }else{
                return eachEmployee
            }
        })
